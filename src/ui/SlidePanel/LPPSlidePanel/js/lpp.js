var lpp = {
    ...
    attr: function(el, propertyName, value){
        if (arguments.length === 2){
            if (!this.isNothing(el[propertyName])){
     ...
     *      pos{string} 定位：relative,absolute
    ...
    hide: function(config){
        ...
        if (_time){
            ...
            for(var i = _time; i >= 1; --i){
                setTimeout(this.createCallback(function(lpp, el, t, time, isHide, afterHide){
                    lpp.css(el, "opacity", time/t);
                    lpp.css(el, "filter", "alpha(opacity=" + (time/t*100) + ")");
                    if (isHide){
                ...
                }, this, _el, _t, i, i === 1, _afterHide), 1 + _time - i);
            }
        }
        else{
